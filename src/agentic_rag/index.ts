 ...
 * 重建 ES 索引并 bulk 写入
 */
const seedElasticsearch = async (indexName: string, rows: any[]) => {
  try {
    console.log("\n[Elasticsearch]");
 ...
 */
const seedMilvus = async (collectionName: string, rows: any[], emb: any) => {
  try {
    console.log("\n[Milvus]");

    const texts = rows.map((row: any) => `${row.note_title}\n${row.note_body}`);
    console.log("生成向量嵌入...");
    ...
    const insertData = rows.map((row: any, i: number) => ({
      id: row.id,
